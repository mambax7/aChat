...
function XHRConnection() {
    ...
    // Objet XML
    var xmlObj;
    // Type of behavior when loading XML
    var xmlLoad;
    ...
    try {
        ...
        if (debug) {
            alert('Error while trying to create object \nnew XMLHttpRequest()\n\n' + error);
        }
        try {
            ...
            try {
                ...
                if (debug) {
                    alert('Eerror while trying to create the object \nnew ActiveXObject("Msxml2.XMLHTTP")\n\n' + error);
                }
                conn = false;
            }
        }
    ...
    // + ----------------------------------------------------------------------------------
    // + setDebugOff
    // + Disables the display of exceptions
    // + ----------------------------------------------------------------------------------
    ...
    // + ----------------------------------------------------------------------------------
    // + Permet de définir l'objet XML qui doit etre valorisé lorsque l'objet XHR reoit une réponse
    // + ----------------------------------------------------------------------------------
    this.setXMLObject = function (obj) {
        if (obj == undefined) {
            if (debug) {
                alert('Paramctre manquant lors de l\'appel de la méthode setXMLObject');
            }
            return false;
        }
        try {
    ...
    // + ----------------------------------------------------------------------------------
    // + sendAndLoad
    // + Connexion r la page désirée avec envoie des données, puis mise en attente de la réponse
    // + Entrées
        ...
        conn.onreadystatechange = function () {
            if (conn.readyState == 4) {
                if (conn.status == 200) {
                    ...
                    // Si une zone destinée r récupérer le résultat a été définie
                    else if (areaId.length > 0) {
                        try {
                            document.getElementById(areaId).innerHTML = conn.responseText;
                        } catch (error) {
                            if (debug) {
                                alert('Echec, ' + areaId + ' n\'est pas un objet valide');
                            }
                        }
                        return;
                    }
        ...
        switch (httpMode) {
            case "GET":
                try {
                    ...
                    if (debug) {
                        alert('Echec lors de la transaction avec ' + Url + ' via la méthode GET');
                    }
                    return false;
                }
                break;
            case "POST":
                try {
                    ...
                    if (debug) {
                        alert('Echec lors de la transaction avec ' + Url + ' via la mthode POST');
                    }
                    return false;
                }
                break;
