...
namespace XoopsModules\Achat\Common;
...
class DirectoryChecker
{
     ...
     * @return bool|string
     */
    public static function getDirectoryStatus0($path, $mode = 0777, $redirectFile = null)
    {
        global $pathIcon16;
        $moduleDirName      = basename(dirname(dirname(__DIR__)));
        $moduleDirNameUpper = mb_strtoupper($moduleDirName);
    ...
    public static function getDirectoryStatus($path, $mode = 0777, $languageConstants = [], $redirectFile)
    {
        ...
        if (!@is_dir($path)) {
            $path_status = "<img src='" . \Xmf\Module\Admin::iconUrl('0.png', '16') . "'>" . $path . ' ( ' . $languageConstants[1] . ' ) ' . '<a href=' . $_SERVER['PHP_SELF']
                           . "?op=createdir&amp;path=$path&amp;redirect=$redirectFile&amp;languageConstants=$myWords1>" . $languageConstants[2] . '</a>';
        } elseif (@is_writable($path)) {
            $path_status = "<img src='" . \Xmf\Module\Admin::iconUrl('1.png', '16') . "'>" . $path . ' ( ' . $languageConstants[0] . ' ) ';
            $currentMode = substr(decoct(fileperms($path)), 2);
            if ($currentMode != decoct($mode)) {
                $path_status = "<img src='" . \Xmf\Module\Admin::iconUrl('0.png', '16') . "'> " . $path . sprintf($languageConstants[3], decoct($mode), $currentMode) . '<a href=' . $_SERVER['PHP_SELF']
                               . "?op=setperm&amp;mode=$mode&amp;path=$path&amp;redirect=$redirectFile&amp;languageConstants=$myWords2> " . $languageConstants[4] . '</a>';
            }
        } else {
            $currentMode = substr(decoct(fileperms($path)), 2);
            $path_status = "<img src='" . \Xmf\Module\Admin::iconUrl('0.png', '16') . "'> " . $path . sprintf($languageConstants[3], decoct($mode), $currentMode) . '<a href=' . $_SERVER['PHP_SELF']
